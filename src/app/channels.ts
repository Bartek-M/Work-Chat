...
form.on("submit", async (e) => {
    ...
    if (!groupName || !selectedMembers.length) return
...
export async function openChannel(channelId: string) {
            ...
            <div class="container-fluid ps-2 justify-content-between">
                ...
                <div class="dropstart" data-bs-toggle="dropdown" aria-expanded="false">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end p-2">
                        ...
                        ${!currentChannel.direct ? `<li class="border-top pt-1 mt-1"><button class="dropdown-item d-flex justify-content-between align-items-center gap-3 px-2 rounded danger-dropdown-btn" id="leave-channel">
                            Wyjdź
                            <svg width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
    ...
    $("#leave-channel").on("click", () => leaveChannel(currentChannel))
...
async function leaveChannel(channel: any) {
    await fetch(`/api/channels/${channel.id}/leave/`, {
        method: "DELETE",
        headers: {
            "X-CSRFToken": (window as any)["csrf"]
        }
        ...
        showToast("API", "Nie udało się opuścić kanału", "error")
    }).catch(() => {
        showToast("API", "Nie udało się opuścić kanału", "error")
    })
}